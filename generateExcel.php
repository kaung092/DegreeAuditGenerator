<?php
		if(isset($_GET['text']))
		{
			...
			$output.=$beg_s.$row['class'].$end."\n";
				...
				if($row['grade']=="W" || $row['grade']=="F" || $row['grade']=="#F")
				{}
				else
				{
						if($Cat == 'MATHEMATICS' )
						{
							if($row['grade']=="CR" || $row['grade']=="IP")
							{
								$output_M.array_push($output_M,$output);
								asort($output_M);
							}
							else if(isset($F_grade) && strnatcmp ($row['grade'],$F_grade ) < 0) // if grade greater than failing grade
							{
								$output_M.array_push($output_M,$output);
								asort($output_M);
							}
						...
						if($Cat == 'LECTURE ELECTIVES')
						{
							if(count($output_LE)<10) //show only 10 courses
							{
